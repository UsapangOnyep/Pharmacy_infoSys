  ...
  if (stocks.length === 0) {
    ...
    stocks.forEach((stock) => {
      ...
      row.innerHTML = `
                  ...
                  <td><img src="${stock.ItemPath}" style="max-width: 100px; height: auto;"></td>
                  <td>${stock.Barcode}</td>
...
function AddItem(event) {
  ...
  const noExpirationChecks = document.querySelectorAll('input[name="noExpiration[]"]');
  ...
  for (let i = 0; i < itemSelects.length; i++) {
    ...
    if (!itemID || !Barcode || !QTY || !Price || (!noExpiration && !ExpiryDate)) {
      Swal.fire({
        icon: "error",
        title: "Error",
...
/* ###################################################
  TABLE TABLE TABLE TABLE TABLE TABLE TABLE TABLE
   ...
   function addRow() {
    let table = document.getElementById("stocksTable").getElementsByTagName("tbody")[0];
    ...
    newRow.innerHTML = `
        ...
        <td><input type="number" name="Price[]" required step="0.01" min="0" value="0" /></td>
...
function toggleExpiryDate(checkbox) {
    let expiryInput = checkbox.closest("tr").querySelector('input[name="ExpiryDate[]"]');
...
function setSelectedItemInfo(select) {
  ...
  fetch(`Actions/Items/fetchItemById.php?id=${selectedItemID}`)
      .then(response => response.json())
      .then(data => {
          row.querySelector('textarea[name="ItemLongDesc[]"]').value = data.ItemDesc;
